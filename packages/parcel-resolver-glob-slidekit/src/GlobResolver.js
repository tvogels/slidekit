...
export default new Resolver({
    async resolve({ dependency, options, specifier, pipeline }) {
        ...
        } else if (dependency.specifierType === "url" && !dependency.meta?.isCSSImport) {
        ...
        return {
            filePath: path.join(
                dir,
                path.basename(specifier, path.extname(specifier)) + "." + sourceAssetType
            ),
            code,
            invalidateOnFileCreate: [{ glob: normalized }],
            invalidateOnFileChange: results.map((r) => r[0]),
            pipeline: null,
            priority: "sync",
        };
    },
...
function generate(matches, isAsync, indent = "", count = 0) {
    ...
    for (let key in matches) {
        ...
        let { imports: i, value, count: c } = generate(matches[key], isAsync, indent + "  ", count);
        imports += `${i}\n`;
        count = c;
