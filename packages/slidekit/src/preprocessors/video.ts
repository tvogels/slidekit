export default function (dom: HTMLElement) {
    for (let node of [...dom.querySelectorAll("[video]")]) {
        ...
        foreignObject.innerHTML = `<video width="${node.getAttribute("width")}" height="${node.getAttribute("height")}" src="${node.getAttribute("video")}"></video>`;
        ...
        for (let { name, value } of node.attributes) {
            if (["stroke", "fill", "stroke-width", "fill-rule", "video", "x", "y"].indexOf(name) >= 0) continue;
